...
class DailySetDurationResourceAdapter: ResourceAdapter<DailySetDuration> {
    ...
    override fun getUpdatedItems(dailySetUid: String, oldVersion: Int): List<DailySetUpdateItem<DailySetDuration>> {
        val dailySetSourceLinks = dailySetSourceLinkMapper.findAllDailySetSourceLinkByDailySetUidAndSourceTypeAndVersionsLargerThan(dailySetUid, DailySetSourceType.Duration.value, oldVersion)
    ...
    private fun join2Sources(dailySetSourceLinks: List<DailySetSourceLinks>, dailySetDurations: List<DailySetDuration>): List<DailySetUpdateItem<DailySetDuration>> {
